...
export default function AdminDashboard() {
  ...
  const exportarExcel = () => {
    ...
    const cols = ['Fecha','Cód. Empleado','Nombre Empleado','Empresa',
    ...
    ws['!cols'] = cols.map((c) => ({ wch: Math.max(c.length, ...filas.map((f) => String(f[c] ?? '').length)) + 2 }))
    const wb = XLSX.utils.book_new()
    const nombreHoja = filtros.mes
      ? `Pedidos ${formatearMes(filtros.mes)}`
      : filtros.regional ? `Pedidos ${filtros.regional}` : 'Pedidos Consolidado'
    XLSX.utils.book_append_sheet(wb, ws, nombreHoja.slice(0, 31))
    const fecha = new Date().toISOString().slice(0, 10)
    const sufijo = filtros.mes ? `_${filtros.mes}` : filtros.regional ? `_${filtros.regional.replace(/ /g, '_')}` : ''
    XLSX.writeFile(wb, `Reporte_PROESA${sufijo}_${fecha}.xlsx`)
  }

  const hayFiltros = filtros.empresa || filtros.regional || filtros.mes || filtros.busqueda
      ...
      <div style={s.historialHeader}>
        <div>
          ...
          <p style={s.caption}>
            ...
            {filtros.mes ? ` · ${formatearMes(filtros.mes)}` : ''}
        ...
        <div style={s.headerActions}>
          ...
          <button onClick={exportarExcel} disabled={!historialFiltrado.length}
              ...
              <tbody>
                {historialPag.map((r, i) => (
                  <tr key={i} style={i % 2 !== 0 ? s.trAlt : {}}>
                    ...
                    <td style={s.td}><span style={s.regionalBadge}>{r.regional}</span></td>
                    <td style={s.td}>
                    ...
                    <td style={s.td}><span style={s.estadoBadge}>{r.estado}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={s.paginacion}>
            ...
            <div style={s.paginaBtns}>
              ...
              {Array.from({ length: Math.min(5, totalPaginas) }, (_, i) => {
                let num
                if (totalPaginas <= 5)              num = i + 1
                else if (pagActual <= 3)             num = i + 1
                else if (pagActual >= totalPaginas - 2) num = totalPaginas - 4 + i
                else                                num = pagActual - 2 + i
                return <PagBtn key={num} onClick={() => setPagina(num)} activo={num === pagActual}>{num}</PagBtn>
              })}
              <PagBtn onClick={() => setPagina((p) => p + 1)} disabled={pagActual === totalPaginas}>›</PagBtn>
              <PagBtn onClick={() => setPagina(totalPaginas)}  disabled={pagActual === totalPaginas}>»</PagBtn>
            </div>
          </div>
        </>
      )}
...
const s = {
  ...
  select: { padding: '0.5rem 0.75rem', border: '1px solid #DDD', borderRadius: 8, fontSize: '0.85rem', background: 'white', minWidth: 140 },
  searchInput: { padding: '0.5rem 0.75rem', border: '1px solid #DDD', borderRadius: 8, fontSize: '0.85rem', minWidth: 220 },
  clearBtn: { alignSelf: 'flex-end', padding: '0.5rem 0.9rem', background: '#F5F5F5', border: '1px solid #DDD', borderRadius: 8, cursor: 'pointer', fontSize: '0.82rem', color: '#666' },
  ...
  exportBtn: { padding: '0.55rem 1.1rem', background: '#1A1A2E', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, alignSelf: 'flex-end' },
  ...
  pagBtn:      { width: 34, height: 34, border: '1px solid #DDD', background: 'white', borderRadius: 7, cursor: 'pointer', fontSize: '0.85rem', color: '#555' },
  pagBtnActivo:{ width: 34, height: 34, border: '1px solid #E63946', background: '#E63946', borderRadius: 7, cursor: 'pointer', fontSize: '0.85rem', color: 'white', fontWeight: 700 },
